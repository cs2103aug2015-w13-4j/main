...
public class Task extends HBox {
	...
	private void initListenerAndFxmlFields(TaskEvent task, int index) {
		...
		initFxmlFields(index, task.getTaskName(), task.getStartDate(), task.getStartTime(), task.getEndDate(),
				task.getEndTime(), task.isCompleted(), task.getPriority(), checkboxListener);
	}
	 ...
	 *            not.
	 */
	private void initFxmlFields(int idx, String taskName, TaskDate sDate, TaskTime startTime, TaskDate eDate,
			TaskTime endTime, Boolean isCompleted, Command_Priority priority,
	 ...
	 *            is the ending time of the task.
	 */
	private void dateAndTimeConfig(TaskDate sDate, TaskTime startTime, TaskDate eDate, TaskTime endTime) {
